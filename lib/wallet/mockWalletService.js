...
export async function createDeposit({ wallet, amount }) {
  const tx = await WalletTransaction.create({
    ...
    direction: "credit",
    note: "Deposit pending confirmations",
    mode: "mock",
  });
  return tx;
}

export async function createOnchainDeposit({ wallet, amount, txHash, fromAddress }) {
  ...
  const tx = await WalletTransaction.create({
    ...
    txHash,
    direction: "credit",
    note: `On-chain deposit from ${String(fromAddress || "").slice(0, 10)}...`,
    mode: "sandbox",
  });
  return tx;
}

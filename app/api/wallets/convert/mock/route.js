...
export async function POST(request) {
  ...
  try {
    const { txOut, usdValue, rate } = await convertToCardUsd({ wallet, amount: parsed.data.amount });
    return NextResponse.json({
      ok: true,
      wallet: serializeWallet(wallet),
      transaction: serializeWalletTransaction(txOut),
      conversion: { rate, usdValue },
    });
  } catch (e) {
    if (e.message === "Insufficient balance" || e.message === "Active virtual card is required") {
      return NextResponse.json({ error: e.message, rate: getMockRate(parsed.data.asset) }, { status: 400 });
    }

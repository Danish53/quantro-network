  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center gap-2 rounded-[10px] border border-white/[0.1] bg-[#1a1f35] px-3 py-2 text-sm font-medium text-slate-200 transition hover:bg-[#232a45]"
        aria-expanded={open}
        aria-haspopup="true"
      >
        ...
        <svg className={`h-4 w-4 text-slate-400 transition ${open ? "rotate-180" : ""}`} viewBox="0 0 20 20" fill="currentColor" aria-hidden>
          <path
            fillRule="evenodd"
      ...
      {open ? (
        <ul
          className="dashboard-sidebar-scroll absolute right-0 z-30 mt-2 max-h-72 w-56 overflow-auto rounded-[10px] border border-white/[0.08] bg-[#161b33] py-1 shadow-xl"
          role="menu"
        >
          {quickLinks.map((item) => (

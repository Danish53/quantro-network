...
function formatBalance(asset, n) {
  const x = Number(n);
...
function truncateAddr(s, left = 10, right = 8) {
  if (!s) return "—";
  if (s.length <= left + right + 1) return s;
  return `${s.slice(0, left)}…${s.slice(-right)}`;
}

export default function AdminCryptoWalletsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/crypto-wallets?${qs}`, { credentials: "include" });
  ...
  return (
    <div className="space-y-6 pb-10">
      <div>
        <h1 className="text-xl font-bold tracking-tight text-slate-100 sm:text-2xl">{t("admin.cw_title")}</h1>
        <p className="mt-1 max-w-2xl text-sm leading-relaxed text-slate-400">{t("admin.cw_subtitle")}</p>
      </div>
      ...
      <div className="flex flex-col gap-3 rounded-[10px] border border-white/[0.08] bg-[#141235]/60 p-4 ring-1 ring-white/[0.04] sm:flex-row sm:flex-wrap sm:items-center sm:justify-between sm:gap-4 sm:p-5">
        <div className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center">
          <input
            ...
            placeholder={t("admin.cw_search_placeholder")}
            ...
            aria-label={t("admin.cw_search_placeholder")}
          />
          <div className="flex shrink-0 items-center gap-2">
            <label htmlFor="admin-cw-asset" className="text-xs font-medium text-slate-500">
              {t("admin.cw_filter_asset")}
            </label>
            <select
              id="admin-cw-asset"
              value={asset}
            ...
            >
              <option value="">{t("admin.cw_asset_all")}</option>
      ...
      {initialTableLoad ? (
        <AdminDataTableSkeleton cols={7} minWidthClass="min-w-[920px]" />
      ) : (
        <div
            ...
            <table className="w-full min-w-[920px] border-collapse text-left text-sm">
              <thead>
                <tr className="border-b border-white/[0.08] bg-[#0F0D2E]/50 text-xs font-semibold uppercase tracking-wide text-slate-500">
                  <th className="whitespace-nowrap px-4 py-3.5 sm:px-5">{t("admin.cw_col_member")}</th>
                  <th className="whitespace-nowrap px-4 py-3.5 sm:px-5">{t("admin.cw_col_account")}</th>
                  <th className="whitespace-nowrap px-4 py-3.5 sm:px-5">{t("admin.cw_col_asset")}</th>
                  <th className="whitespace-nowrap px-4 py-3.5 text-right sm:px-5">{t("admin.cw_col_balance")}</th>
                  <th className="whitespace-nowrap px-4 py-3.5 text-right sm:px-5">{t("admin.cw_col_locked")}</th>
                  <th className="whitespace-nowrap px-4 py-3.5 sm:px-5">{t("admin.cw_col_network")}</th>
                  <th className="whitespace-nowrap px-4 py-3.5 sm:px-5">{t("admin.cw_col_address")}</th>
                </tr>
              </thead>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="whitespace-nowrap px-4 py-3.5 text-right font-mono tabular-nums text-slate-100 sm:px-5">
                          {formatBalance(row.asset, row.balance)}
                        </td>
                        <td className="whitespace-nowrap px-4 py-3.5 text-right font-mono tabular-nums text-slate-400 sm:px-5">
                          {formatBalance(row.asset, row.lockedBalance)}
                        </td>
                        <td className="max-w-[120px] truncate px-4 py-3.5 text-slate-400 sm:px-5" title={row.network}>
                          {row.network}
                        </td>
                        <td
                          className="max-w-[200px] px-4 py-3.5 font-mono text-[11px] text-slate-400 sm:px-5"
                          title={row.address}
                        >

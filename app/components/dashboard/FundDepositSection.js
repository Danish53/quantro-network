...
/**
 * On-chain deposit + recent deposits — same as Fund account page body.
 * @param {{ toastPlacement?: "fixed" | "inline"; embedded?: boolean }} props
 */
export default function FundDepositSection({ toastPlacement = "fixed", embedded = false }) {
  ...
  const sectionGap = embedded ? "mt-6" : "mt-8";
  ...
  return (
    <>
      {toastPlacement === "fixed" ? (
        <div className="pointer-events-none fixed right-4 top-20 z-[80] space-y-2 sm:right-6">{toastBlock}</div>
      ) : (
        <div className="pointer-events-auto relative z-10 mb-6 space-y-2">{toastBlock}</div>
      )}
      ...
      <section className={`${sectionGap} ${sectionClass}`}>
